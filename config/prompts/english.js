/**
 * English-specific prompts for LingoMem
 */
...
function englishRecommendationPrompt(knownWords, stats, count) {
  ...
  return `我正在学习英语，当前水平是 ${levelDescriptions[stats.level] || 'B1（中级）'}，已经学习了 ${stats.totalWords} 个单词。

已学单词列表（请避免推荐这些单词）：
${knownWords.length > 0 ? knownWords.slice(-50).join(', ') : '暂无'}

请根据我的学习进度，推荐 ${count} 个适合我当前水平的英语单词。
...
function englishCustomPrompt(customPrompt, knownWords, count) {
  return `用户需求：${customPrompt}

已学单词列表（请避免推荐这些单词）：
${knownWords.length > 0 ? knownWords.slice(-50).join(', ') : '暂无'}

请根据用户的需求，推荐 ${count} 个合适的英语单词。
